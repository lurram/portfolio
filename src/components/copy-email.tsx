  ...
  return (
    <button
      ...
      className={`
        ...
        font-bold disabled:cursor-wait hover:scale-105 transition-transform active:scale-95 duration-200 shadow-sm
    ...
    >
      ...
      <div className="relative z-10 flex items-center justify-center gap-2">
        {status === 'success' ? (
          ...
          <>
            <IoCopyOutline className={`w-4 h-4 ${status === 'copying' ? 'animate-pulse' : ''}`} />
            <span>{status === 'copying' ? 'Copiando...' : PERSONAL_INFO.email}</span>
          </>
        )}
      </div>
    </button>
